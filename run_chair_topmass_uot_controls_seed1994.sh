...
for item in "${WORKPOINTS[@]}"; do IFS=: read -r a r <<< "$item"; a="$(canon "$a")"; r="$(canon "$r")"; for k in "${TOPKS[@]}"; do out="$(result "$a" "$r" "$k")"; printf 'topmass\talpha%s_rho%s_k%s\t%s\t%s\t%s\t%s\t%s\n' "$a" "$r" "$k" "$SEED" "$a" "$r" "$k" "$out" "${out%.jsonl}_chair.json" >> "$MAN"; complete "$out" || { ALPHA+=("$a"); RHO+=("$r"); K+=("$k"); OUT+=("$out"); ((n+=1)); }; done; done
run() { local i="$1" gpu="$2" out="${OUT[$i]}" stamp; if [[ -f "$out" ]]; then stamp="$(date +%Y%m%d_%H%M%S)"; mv "$out" "${out}.partial.${stamp}"; [[ -f "${out%.jsonl}_ot_stats.jsonl" ]] && mv "${out%.jsonl}_ot_stats.jsonl" "${out%.jsonl}_ot_stats.jsonl.partial.${stamp}"; fi; echo "[GPU $gpu] topmass alpha=${ALPHA[$i]} rho=${RHO[$i]} k=${K[$i]}"; CUDA_VISIBLE_DEVICES="$gpu" "$PYTHON_BIN" chair_eval.py --exp_folder "$EXP" --model "$MODEL" --data-path "$COCO/val2014" --subset-size "$SUBSET_SIZE" --subset-ids-file "$IDS" --seed "$SEED" --max-new-tokens 512 --vsv --vsv-lambda 0.17 --logits-aug --logits-layers 25,30 --logits-alpha "${ALPHA[$i]}" --use-ot-bary-sla --ot-attention-visual-marginal --ot-topk 16 --ot-sinkhorn-iters 100 --ot-sinkhorn-tolerance 0.001 --ot-epsilon 0.05 --ot-layer-temperature 0.06 --ot-attention-power 0.75 --ot-attention-uniform-mix 0.02 --ot-unbalanced --ot-marginal-relaxation "${RHO[$i]}" --ot-mass-aware-layer-weights --ot-direction-aware-gating --ot-independent-uniform-layer-weights --ot-head-aware-mode topmass --ot-head-topk "${K[$i]}" --ot-log-stats > "$SWEEP/logs/topmass_a${ALPHA[$i]}_r${RHO[$i]}_k${K[$i]}.log" 2>&1; complete "$out"; }
echo "Top-M single-UOT control: $n pending jobs on GPUs ${GPUS[*]}"; declare -a PIDS=(); failed=0; for ((i=0;i<n;i++)); do run "$i" "${GPUS[$((i % ${#GPUS[@]}))]}" & PIDS+=("$!"); [[ ${#PIDS[@]} -lt ${#GPUS[@]} ]] || { for p in "${PIDS[@]}"; do wait "$p" || failed=1; done; PIDS=(); }; done; for p in "${PIDS[@]}"; do wait "$p" || failed=1; done; (( failed == 0 )) || { echo "Generation failed; inspect $SWEEP/logs" >&2; exit 1; }

...
echo "Top-M single-UOT control: $n pending jobs on GPUs ${GPUS[*]}"; declare -a PIDS=(); for ((i=0;i<n;i++)); do run "$i" "${GPUS[$((i % ${#GPUS[@]}))]}" & PIDS+=("$!"); [[ ${#PIDS[@]} -lt ${#GPUS[@]} ]] || { for p in "${PIDS[@]}"; do wait "$p"; done; PIDS=(); }; done; for p in "${PIDS[@]}"; do wait "$p"; done

...
TRACE_DATA_ROOT="${RETRIEVAL_SHIFT_DATA_ROOT:-/data/sun_yuxi/retrieval_shift_data}"
COMPACT="${COMPACT:-1}"
...
TRACE_ROOT="$TRACE_DATA_ROOT/$RUN_NAME"
mkdir -p "$OUT/manifests" "$OUT/logs" "$TRACE_ROOT"
...
echo "Heavy per-sample traces are sharded under $TRACE_ROOT (compact=$COMPACT)."
echo "Repository-local manifests, logs, and caption outputs remain under $OUT."
echo "Do not build a monolithic summary for 200+ samples."

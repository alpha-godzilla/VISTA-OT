#!/usr/bin/env bash
# Seed/GT-free first-stage UOT ablation. Seeds select paired evaluation images
# only; no labels, calibration split, or result-based parameter selection is used.
set -Eeuo pipefail
...
read -r -a MARGINAL_RELAXATIONS <<< "${MARGINAL_RELAXATIONS:-0.5}"
...
SWEEP_DIR="${SWEEP_DIR:-$SCRIPT_DIR/exp_results/chair_ot_direction_aware_uot_ablation}"
VISTA_EXP_FOLDER="${VISTA_EXP_FOLDER:-chair_ot_direction_aware_uot_vista}"
OT_EXP_FOLDER="${OT_EXP_FOLDER:-chair_ot_direction_aware_uot_otattn}"
...
if (( ${#SEEDS[@]} == 0 || ${#GPUS[@]} == 0 || ${#MARGINAL_RELAXATIONS[@]} == 0 )); then
  echo "SEEDS, GPU_IDS, and MARGINAL_RELAXATIONS must be non-empty." >&2
  exit 1
fi
...
declare -a JOB_METHODS=() JOB_SETTINGS=() JOB_SEEDS=() JOB_IDS=() JOB_RESULTS=() JOB_RELAXATIONS=()
pending=0
enqueue() {
  local method="$1" setting="$2" seed="$3" ids="$4" result="$5" relaxation="$6" gpu
  if is_complete "$result"; then
    gpu=-1
  else
    gpu="${GPUS[$((pending % ${#GPUS[@]}))]}"
    JOB_METHODS+=("$method"); JOB_SETTINGS+=("$setting"); JOB_SEEDS+=("$seed")
    JOB_IDS+=("$ids"); JOB_RESULTS+=("$result"); JOB_RELAXATIONS+=("$relaxation")
    ((pending += 1))
  fi
...
for seed in "${SEEDS[@]}"; do
  ...
  enqueue balanced_ot base "$seed" "$ids" "$(ot_result "$seed" '')" 0
  for raw_relaxation in "${MARGINAL_RELAXATIONS[@]}"; do
    relaxation="$(canonical_float "$raw_relaxation")"
    "$PYTHON_BIN" -c 'import sys; assert float(sys.argv[1]) > 0' "$relaxation" || { echo "Each marginal relaxation must be positive." >&2; exit 1; }
    enqueue naive_uot "rho${relaxation}" "$seed" "$ids" "$(ot_result "$seed" "_uot_mrel${relaxation}")" "$relaxation"
    enqueue mass_aware_uot "rho${relaxation}" "$seed" "$ids" "$(ot_result "$seed" "_uot_mrel${relaxation}_masslayer")" "$relaxation"
    enqueue direction_aware_uot "rho${relaxation}" "$seed" "$ids" "$(ot_result "$seed" "_uot_mrel${relaxation}_masslayer_dirgate")" "$relaxation"
  done
done

run_job() {
  local index="$1" gpu="$2"
  local method="${JOB_METHODS[$index]}" setting="${JOB_SETTINGS[$index]}" seed="${JOB_SEEDS[$index]}"
  local ids="${JOB_IDS[$index]}" result="${JOB_RESULTS[$index]}" relaxation="${JOB_RELAXATIONS[$index]}"
  ...
  if [[ "$method" == vista ]]; then
    ...
    if [[ "$method" != balanced_ot ]]; then
      args+=(--ot-unbalanced --ot-marginal-relaxation "$relaxation")
    fi
    if [[ "$method" == mass_aware_uot || "$method" == direction_aware_uot ]]; then
      args+=(--ot-mass-aware-layer-weights)
    fi
    if [[ "$method" == direction_aware_uot ]]; then
      args+=(--ot-direction-aware-gating)
    fi
  fi
  if [[ -f "$result" ]]; then
...
echo "Direction-aware UOT ablation: ${#JOB_METHODS[@]} pending jobs on GPUs ${GPUS[*]}"
declare -a PIDS=()
...
echo "Direction-aware UOT ablation complete: $SWEEP_DIR/summary.md"
